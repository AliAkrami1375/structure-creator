...
var args = parseArgs(Argument.slice(2), {
    ...
    boolean: ["help", "version", "socketio", "auth", "menu"],
    default: {
        port: 3000,
        path: ".",
        lang: 'en'
    ...
    var ListOfPrompt = [
        {
            type: "list",
            name: "type",
            message: "select type structure",
            choices: ["express", "loadbalancer","go","flask","vue","nuxt"]
        }
    ]
    inquirer
    .prompt(ListOfPrompt)
    .then((answers) => {
        switch (answers.type) {
            case "express":
                ...
                break;
            default:
                console.log("coming soon".green);
                break;
        }
    });
}

function ExpressMenu(){
    var ListOfPrompt = [
        {
            type: "input",
            name: "path",
            default:".",
            message: "Enter Path Project :"
        },
        {
            type: "input",
            name: "port",
            default:".",
            message: "Enter Port Project :"
        },
        {
            type: "input",
            name: "lang",
            default:"en",
            message: "Enter languages api Project :"
        },
        {
            type: "confirm",
            name: "auth",
            message: "Is Auth Enable?",
            default:false
        },
        {
            type: "confirm",
            name: "socket",
            message: "Is Socketio Enable?",
            default:false
        }
    ]
    inquirer
    .prompt(ListOfPrompt)
    .then((answers) => {
        args.auth = answers.auth;
        args.socketio = answers.socket;
        args.port = answers.port;
        args.P = answers.port;
        args.p = answers.path;
        args.path = answers.path;
        args.lang = answers.lang;
        args.l = answers.lang;
        Loading();
    });
}
...
function usage() {
    ...
    console.log('  Options:')
    console.log('')
    console.log('    -m, --menu              wizard creator')
